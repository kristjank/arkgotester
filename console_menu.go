...
func printMenu() {
	...
	color.Set(color.FgHiYellow)
	fmt.Println("")
	fmt.Println("\t1-Send transfer - type 0 [", viper.GetInt("env.txPerPayload"), "/", viper.GetInt("env.txIterations"), "]")
	fmt.Println("\t2-Send transfer with random fees [", viper.GetInt("env.dynamicFeeMin"), "--", viper.GetInt("env.dynamicFeeMax"), "]")
	fmt.Println("\t3-Send vote for delegate")
